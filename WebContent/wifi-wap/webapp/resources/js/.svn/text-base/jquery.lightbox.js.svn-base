		...
		*/
		function showLightbox(imageObject) {
			...
			**/
			...
			changeImage(imageNum,imageObject);
		};
	    
		function changeImage(imageNum,imageObject) {
			if (opts.inprogress == false) {
				...
				doChangeImage(imageObject);
			}
		};

		function doChangeImage(imageObject) {
			...
			imgPreloader.onload = function() {
				...
				imgPreloader.src = imageObject.src;
			};
		...
		$.fn.lightbox.defaults = {
		  triggerEvent: "click",
			...
			bottomNavClose: false,
			jsonData: null,
			jsonDataParser: null
		};	
})(jQuery);

 function addLigthBoxEvent(){//获取头像方法
 	$(".lightbox").lightbox({
			    fitToScreen: true,
		        imageClickClose: true,
		        displayTitle: false
