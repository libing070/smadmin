...
artDialog.fn = artDialog.prototype = {
    ...
    constructor: function (config) {
		...
		//
		this.titleClose(config.titleClose);
		//
        ...
        this.zIndex();
        config.lock && this.lock(config.isNotClick,config.divId);
    ...
    */
    ...
    /**
	 * 标题 行 关闭按钮是否展示，true为不展示，其他为展示
	 */
	titleClose: function(flag) {
		if (flag) {
            this.dom.close.hide();
        }
	},
    ...
    */
     ...
     */
     lock: function (isNotClick,divId) {
        ...
        var _height_ = $(document).height();
        var _width_default = "100%";
        var _height_default = "100%";
        if(divId != null && divId != "" && divId != undefined){
            _width_ = divId.width();
            _height_ = divId.height();
            _width_default = divId.width();
            _height_default = divId.height();
            _top_ = divId.offset().top;
            _left_ = divId.offset().left;
        }
        $div.css({
            zIndex: index,
            position: 'fixed',
...
31. 增加titleClose 参数，用于设置title部分的关闭按钮是否展示，设置为true不展示，默认展示
32. lock方法中，增加了isNotClick 属性判断，如果默认为true,支持lock遮罩层的双击关闭，false时，则不支持双击关闭
*/
