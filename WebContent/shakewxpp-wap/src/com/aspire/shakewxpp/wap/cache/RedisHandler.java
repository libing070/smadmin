...
//			logger.info("redis配置文件 path=" + fileName);
	 ...
	 * @return
	 */
	public synchronized long synPushObjectForNum(long addNum, String key, String str,long maxElement,int sec) throws Exception{
		Jedis jedis = null;
		long addResultNum = 0;
		try {
			jedis = this.getJedis();
			...
			if(maxElement-length<addNum){
				addNum = maxElement-length;
			}
	 ...
	 * @return
	 */
	public synchronized boolean synPushObject(String key, String str,long maxElement,int sec) throws Exception{
		Jedis jedis = null;
		boolean flag = true;
		try {
			jedis = this.getJedis();
	 ...
	 * @return
	 */
	public synchronized long synPushObjectT(String key, String str,long maxElement,int sec) throws Exception{
		Jedis jedis = null;
		long flag = 0;
		try {
			jedis = this.getJedis();
	 ...
	 * @return
	 */
	public synchronized boolean popObjectT(String key) throws Exception{
		Jedis jedis = null;
		boolean flag = true;
		try {
			jedis = this.getJedis();
			...
			flag = false;
			logger.error("popObject failed. key=" + key, e);
			pool.returnBrokenResource(jedis);
			throw e;
		} finally {
			pool.returnResource(jedis);
